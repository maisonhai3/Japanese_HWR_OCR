...
class Noiser(object):
    ...
    def apply(self, img):
        ...
        if self.cfg.noise.ver_noise.enable:
            p.append(self.cfg.noise.ver_noise.fraction)
            funcs.append(self.apply_ver_noise)
    ...
    def apply_sp_noise(self, img):
        ...
        first_point = img[0,0]
        s_vs_p = 0.3 # 0.5
        amount = np.random.uniform(0.01, 0.2)
        out = np.copy(img)
        # Salt mode
        num_salt = np.ceil(amount * img.size * s_vs_p)
        coords = [np.random.randint(0, i - 1, int(num_salt))
                  for i in img.shape[:2]]
        out[coords] = first_point
    ...
    def apply_ver_noise(self, img):
        ...
        for i in range(1, n):
            ...
            w_noise = col + np.random.randint(1, 4)
            w_noise = w_noise if w_noise <= w else w
            out[:, col:w_noise] = first_point
