...
from textrenderer.hline_extraction import text_2_image
...
class Renderer(object):
    def __init__(self, corpus, fonts, bgs, cfg, width=256, height=32,
        ...
        self.strict = strict
        self.cfg = cfg
        self.char_label = pd.read_csv(self.cfg.char_label.path)
    ...
    def gen_img(self, img_index):
        ...
        # gen handwritten line
        word, font, word_size = self.pick_font(img_index)
        text_box_pnts = None
        while text_box_pnts is None:
            word_img, text_box_pnts, word_color = text_2_image(word, self.cfg, self.char_label, font)
        ...
        if apply(self.cfg.top_noise) and not apply(self.cfg.crop):
            word_img = self.adding_top_noise(word_img, self.cfg.top_noise)

        if apply(self.cfg.bottom_noise) and not apply(self.cfg.crop):
            word_img = self.adding_bottom_noise(word_img, self.cfg.bottom_noise)

        if apply(self.cfg.fx) and not apply(self.cfg.crop):
            word_img = self.decrease_ratio(word_img, self.cfg.fx)
    ...
    def adding_top_noise(self, word_img, top_noise_cfg):
        if prob(top_noise_cfg.fraction):
            h, w, c = word_img.shape
            padding_percent = 0.15
            h_padding = random.randint(0, int(padding_percent * h))

            y_top = int(h/2 - h_padding/2)
            y_bot = int(h/2 + h_padding/2)
            h_padding = y_bot - y_top
            roi_img = word_img[y_top:y_bot, :]

            clone_image = word_img.copy()
            clone_image = cv2.copyMakeBorder(clone_image, h_padding, 0, 0, 0, cv2.BORDER_REPLICATE)
            clone_image[:h_padding, :] = roi_img
            return clone_image
        else: return word_img

    def adding_bottom_noise(self, word_img, bottom_noise_cfg):
        if prob(bottom_noise_cfg.fraction):
            h, w, c = word_img.shape
            padding_percent = 0.15
            h_padding = random.randint(0, int(padding_percent * h))
            ...
            clone_image = word_img.copy()
            clone_image = cv2.copyMakeBorder(clone_image, 0, h_padding, 0, 0, cv2.BORDER_REPLICATE)
            clone_image[h:h + h_padding, :] = roi_img
            return clone_image
        else: return word_img

    def decrease_ratio(self, word_img, fx_config):
        if prob(fx_config.fraction):
            fx = random.uniform(fx_config.fx_min, fx_config.fx_max)
            return cv2.resize(word_img, dsize=None, fx=fx, fy = 1.0)
        return word_img